...
func TestScript(t *testing.T) {
	for name, cas := range scriptCases {
		t.Run(name, func(t *testing.T) {
			...
			script, err := scriggo.LoadScript(bytes.NewReader([]byte(cas.src)), cas.pkgs, scriggo.LoadOptions{})
			if err != nil {
...
func TestScriptSum(t *testing.T) {
	...
	script, err := scriggo.LoadScript(bytes.NewReader([]byte(src)), pkgs, scriggo.LoadOptions{})
	if err != nil {
...
func TestScriptChainMessages(t *testing.T) {
	...
	script1, err := scriggo.LoadScript(bytes.NewReader([]byte(src1)), pkgs, scriggo.LoadOptions{})
	if err != nil {
		t.Fatalf("unable to load script 1: %s", err)
	}
	script2, err := scriggo.LoadScript(bytes.NewReader([]byte(src2)), pkgs, scriggo.LoadOptions{})
	if err != nil {
