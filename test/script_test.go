...
func TestScript(t *testing.T) {
	for name, cas := range scriptCases {
		t.Run(name, func(t *testing.T) {
			...
			err = script.Run(nil, cas.init, scriggo.RunOptions{})
			if err != nil {
...
func TestScriptSum(t *testing.T) {
	...
	err = script.Run(nil, init, scriggo.RunOptions{})
	if err != nil {
		t.Fatalf("run: %s", err)
	}
	if Sum != 45 {
...
func TestScriptChainMessages(t *testing.T) {
	...
	err = script1.Run(nil, init, scriggo.RunOptions{})
	if err != nil {
		t.Fatalf("run: %s", err)
	}
	...
	err = script2.Run(nil, init, scriggo.RunOptions{})
	if err != nil {
		t.Fatalf("run: %s", err)
	}
