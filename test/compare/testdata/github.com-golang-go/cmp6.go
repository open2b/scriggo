...
func main() {
	...
	use(t3 == t3) // ERROR `cannot be compared`
	...
	var m map[int]int
	_ = m
	use(x == x) // ERROR `[]int cannot be compared`
	use(f == f) // ERROR `invalid operation: f == f (func() cannot be compared)`
	use(m == m) // ERROR `invalid operation: m == m (map[int]int cannot be compared)`
