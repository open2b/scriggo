...
func TestShiftOfZero() {
	if got := ofz64l64(5); got != 0 {
		errorf("0<<5 == %d, want 0", got)
	}
...
func TestShiftCombine() {
	if got, want := two64l(4), int64(16); want != got {
		errorf("4<<1<<1 == %d, want %d", got, want)
	}
	if got, want := two64r(64), int64(16); want != got {
		errorf("64>>1>>1 == %d, want %d", got, want)
	}
...
func TestShiftLargeCombine() {
	var N uint64 = 0x8000000000000000
	if one64<<N<<N == 1 {
		errorf("shift overflow mishandled")
	}
	if one64>>N>>N == 1 {
		errorf("shift overflow mishandled")
	}
