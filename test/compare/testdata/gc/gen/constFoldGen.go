...
func main() {
	...
	for _, s := range szs {
		for _, o := range ops {
			...
			tests = append(tests, fmt.Sprintf("testConstFold%s%s", s.name, o.name))
			fmt.Fprintf(w, "func testConstFold%s%s() {\n", s.name, o.name)
			...
			for _, c := range s.u {
				...
				for _, d := range s.u {
					...
					fmt.Fprintf(w, "\t\tlog.Fatalf(\"%d %%s %d = %%d, want %s\", %q, r)\n", c, d, want, o.symbol)
					fmt.Fprintf(w, "\t}\n")
				}
			}
			// signed test cases
	...
	for _, ls := range szs {
		for _, rs := range szs {
			...
			for _, o := range ops {
				...
				tests = append(tests, fmt.Sprintf("testConstFold%s%s%s", ls.name, rs.name, o.name))
				fmt.Fprintf(w, "func testConstFold%s%s%s() {\n", ls.name, rs.name, o.name)
	...
	for _, s := range szs {
		tests = append(tests, fmt.Sprintf("testConstFoldCompare%s", s.name))
		fmt.Fprintf(w, "func testConstFoldCompare%s() {\n", s.name)
		for _, x := range s.i {
			for _, y := range s.i {
				fmt.Fprintf(w, "\t{\n")
				...
				if x == y {
					fmt.Fprintf(w, "\t\tif !(x == y) { log.Fatalf(\"!(%%d == %%d)\", x, y) }\n")
				} else {
					fmt.Fprintf(w, "\t\tif x == y { log.Fatalf(\"%%d == %%d\", x, y) }\n")
				}
				if x != y {
	...
	// write to file
	err = ioutil.WriteFile("../constFold.go", src, 0666)
	if err != nil {
