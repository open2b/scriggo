...
// Test results for comparison operations against constants.
func main() {
	for i, test := range uint64_tests {
		for j, x := range uint64_vals {
			...
			if test.fn(x) != want {
				...
				log.Fatalf("test failed: %v(%v) != %v [type=uint64 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range uint32_tests {
		for j, x := range uint32_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=uint32 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range uint16_tests {
		for j, x := range uint16_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=uint16 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range uint8_tests {
		for j, x := range uint8_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=uint8 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range int64_tests {
		for j, x := range int64_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=int64 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range int32_tests {
		for j, x := range int32_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=int32 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range int16_tests {
		for j, x := range int16_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=int16 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
	for i, test := range int8_tests {
		for j, x := range int8_vals {
			...
			if test.fn(x) != want {
				fn := runtime.FuncForPC(reflect.ValueOf(test.fn).Pointer()).Name()
				log.Fatalf("test failed: %v(%v) != %v [type=int8 i=%v j=%v idx=%v]", fn, x, want, i, j, test.idx)
			}
		}
	}
}
