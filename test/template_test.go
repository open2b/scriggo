...
func TestTemplate(t *testing.T) {
	for name, cas := range templateCases {
		t.Run(name, func(t *testing.T) {
			...
			templ, err := template.Load("/main", r, builtins, template.ContextText, template.LoadOption{})
			if err != nil {
...
func TestMultiPageTemplate(t *testing.T) {
	for name, cas := range templateMultiPageCases {
		t.Run(name, func(t *testing.T) {
			...
			templ, err := template.Load("/index.html", r, template.Builtins(), template.ContextText, template.LoadOption{})
			if err != nil {
				t.Fatalf("loading error: %s", err)
			}
			w := &bytes.Buffer{}
