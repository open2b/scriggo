...
func newConstantInt(n *big.Int) ConstantNumber {
	return ConstantNumber{constant.MakeFromLiteral(n.String(), gotoken.INT, 0), TypeInt}
}
...
func newConstantFloat(n *big.Float) ConstantNumber {
	return ConstantNumber{constant.MakeFromLiteral(n.Text('f', -1), gotoken.FLOAT, 0), TypeFloat64}
}
...
// ToType returns n converted to type typ, or the default type if typ is nil.
// If it is not convertible to typ, returns an errConstantConversion error.
// If there is an overflow returns an errConstantOverflow error.
...
func (n ConstantNumber) BinaryOp(op ast.OperatorType, y ConstantNumber) (interface{}, error) {
	...
	case ast.OperatorEqual:
		v = constant.Compare(n.val, gotoken.EQL, y.val)
	case ast.OperatorNotEqual:
		v = constant.Compare(n.val, gotoken.NEQ, y.val)
	case ast.OperatorLess:
		v = constant.Compare(n.val, gotoken.LSS, y.val)
	case ast.OperatorLessOrEqual:
		v = constant.Compare(n.val, gotoken.LEQ, y.val)
	case ast.OperatorGreater:
		v = constant.Compare(n.val, gotoken.GTR, y.val)
	case ast.OperatorGreaterOrEqual:
		v = constant.Compare(n.val, gotoken.GEQ, y.val)
	case ast.OperatorAddition:
		v = constant.BinaryOp(n.val, gotoken.ADD, y.val)
	case ast.OperatorSubtraction:
		v = constant.BinaryOp(n.val, gotoken.SUB, y.val)
	case ast.OperatorMultiplication:
		v = constant.BinaryOp(n.val, gotoken.MUL, y.val)
	case ast.OperatorDivision:
		if constant.Sign(y.val) == 0 {
			return nil, errDivisionByZero
		}
		if n.typ == TypeFloat64 || y.typ == TypeFloat64 {
			v = constant.BinaryOp(n.val, gotoken.QUO, y.val)
		} else {
	...
	case ast.OperatorModulo:
		...
		v = constant.BinaryOp(n.val, gotoken.REM, y.val)
	}
...
func (n ConstantNumber) Neg() ConstantNumber {
	return ConstantNumber{val: constant.UnaryOp(gotoken.SUB, n.val, 0), typ: n.typ}
}
