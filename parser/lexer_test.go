...
var typeTests = map[string][]tokenType{
	...
	"{% macro \"\" %}":             {tokenStartStatement, tokenMacro, tokenInterpretedString, tokenEndStatement},
...
var contextTests = map[ast.Context]map[string][]ast.Context{
	...
	ast.ContextHTML: {
		...
		`<script>s{{show "a"}}t</script>`:              {ast.ContextText, ast.ContextScript, ast.ContextScript, ast.ContextScript, ast.ContextScript, ast.ContextText},
