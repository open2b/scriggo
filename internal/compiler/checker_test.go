...
// (or empty string if the type checking is valid). Tests are subdivided for
// categories. Each category has a title (indicated by a comment), and it's
// split in two parts: correct source codes (which goes first) and bad ones.
// Correct source codes and bad source codes are, respectively, sorted by
...
var checkerStmts = map[string]string{
	...
	`type T int; _ = struct{ T }{}`:                                               ok,
	`type T int; _ = struct{ *T }{}`:                                              ok,
	`type T *int; _ = struct{ T }{}`:                                              `embedded type cannot be a pointer`,
	`type T *int; _ = struct{ *T }{}`:                                             `embedded type cannot be a pointer`,
	`type T int; type A = T; _ = struct{ A }{}`:                                   ok,
	`type T int; type A = T; _ = struct{ *A }{}`:                                  ok,
	`type T *int; type A = T; _ = struct{ A }{}`:                                  `embedded type cannot be a pointer`,
	`type T *int; type A = T; _ = struct{ *A }{}`:                                 `embedded type cannot be a pointer`,
	`type T int; type A = *T; _ = struct{ A }{}`:                                  ok,
	`type T int; type A = *T; _ = struct{ *A }{}`:                                 `embedded type cannot be a pointer`,
	`type T interface{}; _ = struct{ T }{}`:                                       ok,
	`type T interface{}; _ = struct{ *T }{}`:                                      `embedded type cannot be a pointer to interface`,
	`type T *interface{}; _ = struct{ T }{}`:                                      `embedded type cannot be a pointer`,
	`type T interface{}; type A = T; _ = struct{ A }{}`:                           ok,
	`type T *interface{}; type A = T; _ = struct{ A }{}`:                          `embedded type cannot be a pointer`,
	`type T interface{}; type A = T; _ = struct{ *A }{}`:                          `embedded type cannot be a pointer to interface`,
	`type T interface{}; type A = *T; _ = struct{ A }{}`:                          `embedded type cannot be a pointer to interface`,
	`type T int; type A = T; _ = struct{ A }{}.A`:                                 ok,
	`type T int; type A = *T; _ = struct{ A }{}.A`:                                ok,
	`type T int; type A = T; _ = struct{ *A }{}.A`:                                ok,
	`type T int; type A = T; type B = A; _ = struct{ B }{}.B`:                     ok,
	`type T int; _ = struct{ T int; T }{}`:                                        `duplicate field T`,
	`type t int; _ = struct{ t int; t }{}`:                                        `duplicate field t`,
