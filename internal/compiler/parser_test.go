...
var treeTests = []struct {
	...
	{"{% show a %}", ast.NewTree("", []ast.Node{
		ast.NewShowMacro(p(1, 1, 0, 11), nil, ast.NewIdentifier(p(1, 9, 8, 8), "a"), nil, ast.ContextHTML)}, ast.ContextHTML)},
...
func TestGoContextTrees(t *testing.T) {
	for _, tree := range goContextTreeTests {
		node, _, err := ParseSource([]byte(tree.src), false, false, ast.ContextGo)
		if err != nil {
			t.Errorf("source: %q, %s\n", tree.src, err)
			continue
		}
...
func TestTrees(t *testing.T) {
	for _, tree := range treeTests {
		node, _, err := ParseSource([]byte(tree.src), false, false, ast.ContextHTML)
		if err != nil {
			t.Errorf("source: %q, %s\n", tree.src, err)
			continue
		}
