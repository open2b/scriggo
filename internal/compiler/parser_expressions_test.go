...
var exprTests = []struct {
	...
	{"1<=2", ast.NewBinaryOperator(p(1, 2, 0, 3), ast.OperatorLessEqual, ast.NewBasicLiteral(p(1, 1, 0, 0), ast.IntLiteral, "1"), ast.NewBasicLiteral(p(1, 4, 3, 3), ast.IntLiteral, "2"))},
	...
	{"a and b", ast.NewBinaryOperator(p(1, 3, 0, 6), ast.OperatorExtendedAnd,
		ast.NewIdentifier(p(1, 1, 0, 0), "a"),
		ast.NewIdentifier(p(1, 7, 6, 6), "b")),
	},
	{"a or b", ast.NewBinaryOperator(p(1, 3, 0, 5), ast.OperatorExtendedOr,
		ast.NewIdentifier(p(1, 1, 0, 0), "a"),
		ast.NewIdentifier(p(1, 6, 5, 5), "b"),
	)},
	{"a or not b", ast.NewBinaryOperator(p(1, 3, 0, 9),
		ast.OperatorExtendedOr,
		ast.NewIdentifier(p(1, 1, 0, 0), "a"),
		ast.NewUnaryOperator(
			p(1, 6, 5, 9),
			ast.OperatorExtendedNot,
	...
	{"a not contains b", ast.NewBinaryOperator(p(1, 3, 0, 15), ast.OperatorNotContains,
		ast.NewIdentifier(p(1, 1, 0, 0), "a"),
		ast.NewIdentifier(p(1, 16, 15, 15), "b")),
	},
	{"a default b", ast.NewDefault(p(1, 3, 0, 10),
		ast.NewIdentifier(p(1, 1, 0, 0), "a"),
		ast.NewIdentifier(p(1, 11, 10, 10), "b")),
	},
	{"M() default b", ast.NewDefault(p(1, 5, 0, 12),
		ast.NewCall(p(1, 2, 0, 2), ast.NewIdentifier(p(1, 1, 0, 0), "M"), nil, false),
		ast.NewIdentifier(p(1, 13, 12, 12), "b")),
	},
	{"render \"a\" default b", ast.NewDefault(p(1, 12, 0, 19),
		ast.NewRender(p(1, 1, 0, 9), "a"),
		ast.NewIdentifier(p(1, 20, 19, 19), "b")),
	},
	{"a + b default c", ast.NewBinaryOperator(p(1, 3, 0, 14), ast.OperatorAddition,
		ast.NewIdentifier(p(1, 1, 0, 0), "a"), ast.NewDefault(p(1, 7, 4, 14),
			ast.NewIdentifier(p(1, 5, 4, 4), "b"),
			ast.NewIdentifier(p(1, 15, 14, 14), "c"))),
	},
	{"[]int{} and !x.F", ast.NewBinaryOperator(
		p(1, 9, 0, 15),
		ast.OperatorExtendedAnd,
	...
	{"func()", ast.NewFuncType(p(1, 1, 0, 5), false, nil, nil, false)},
	...
	{"func(a int, b ...int) {}",
		ast.NewFunc(p(1, 1, 0, 23), nil, ast.NewFuncType(p(1, 1, 0, 23), false, []*ast.Parameter{
			ast.NewParameter(ast.NewIdentifier(p(1, 6, 5, 5), "a"), ast.NewIdentifier(p(1, 8, 7, 9), "int")),
			ast.NewParameter(ast.NewIdentifier(p(1, 13, 12, 12), "b"), ast.NewIdentifier(p(1, 18, 17, 19), "int")),
		}, nil, true), ast.NewBlock(p(1, 23, 22, 23), nil), false, ast.FormatText)},
	{"func(p.T) {}",
	...
	{"func(a ...p.T) {}",
		...
		}, nil, true), ast.NewBlock(p(1, 15, 15, 16), nil), false, ast.FormatText),
	},
}
