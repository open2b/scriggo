...
func (em *emitter) emitNodes(nodes []ast.Node) {
	...
	for _, node := range nodes {
		...
		case *ast.Show:
			...
			em.fb.emitMove(true, int8(node.Context), em.fb.templateRegs.iA, reflect.Int, true)
			if em.inURL {
				// In a URL context: use the urlWriter, that implements io.Writer.
				em.fb.emitMove(false, em.fb.templateRegs.gF, em.fb.templateRegs.gD, reflect.Interface, true)
			} else {
				// Not in a URL context: use the default writer.
				em.fb.emitMove(false, em.fb.templateRegs.gA, em.fb.templateRegs.gD, reflect.Interface, true)
			}
	...
	for i, cas := range selectNode.Cases {
		...
		if assignment, isAssignment := cas.Comm.(*ast.Assignment); isAssignment {
			...
			if len(assignment.Lhs) == 2 { // case has 'ok'
				em.fb.emitMove(true, 1, ok, reflect.Bool, true)
				em.fb.emitIf(false, 0, runtime.ConditionOK, 0, reflect.Interface, assignment.Pos())
				em.fb.emitMove(true, 0, ok, reflect.Bool, true)
