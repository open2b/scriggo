			...
			if em.isTemplate {
				if node.Ident != nil && node.Ident.Name == "_" {
					...
					if em.scriggoPackageVars[backupPkg] == nil {
						em.scriggoPackageVars[backupPkg] = map[string]int16{}
					}
					for name, v := range vars {
						if importName == "" {
							em.scriggoPackageVars[backupPkg][name] = v
						} else {
							em.scriggoPackageVars[backupPkg][importName+"."+name] = v
						}
					}
					if len(inits) > 0 {
...
func (em *emitter) emitAssignmentNode(node *ast.Assignment) {
	...
	for i, v := range node.Lhs {
		...
		case *ast.Identifier:
			...
			if index, ok := em.nonLocalVarIndex(v); ok {
