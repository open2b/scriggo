...
func (c *Compiler) compileCall(call *ast.Call) ([]int8, []reflect.Type) {
	...
	var funReg int8
	out, _, isRegister := c.quickCompileExpr(call.Func, c.typeinfo[call.Func].Type)
	if isRegister {
		funReg = out
	} else {
...
func (c *Compiler) compileExpr(expr ast.Expression, reg int8, dstType reflect.Type) {
	...
	case *ast.BinaryOperator:
		...
		var ky bool
		{
			out, isValue, isRegister := c.quickCompileExpr(expr.Expr2, typ)
			if isValue {
				op2 = out
				ky = true
			} else if isRegister {
	...
	case *ast.CompositeLiteral:
		...
		case reflect.Slice, reflect.Array:
			...
			for _, kv := range expr.KeyValues {
				...
				var kvalue bool
				var value int8
				out, isValue, isRegister := c.quickCompileExpr(kv.Value, typ.Elem())
				if isValue {
					value = out
					kvalue = true
				} else if isRegister {
	...
	case *ast.TypeAssertion:
		...
		out, _, isRegister := c.quickCompileExpr(expr.Expr, typ)
		var exprReg int8
		if isRegister {
			exprReg = out
		} else {
...
func (c *Compiler) compileVariableAssignment(variable ast.Expression, value int8, valueType reflect.Type, kvalue bool, isDecl bool) {
	...
	case *ast.Index:
		...
		case reflect.Slice:
			var slice int8
			out, _, isRegister := c.quickCompileExpr(variable.Expr, c.typeinfo[variable.Expr].Type)
			if isRegister {
				slice = out
			} else {
			...
			var index int8
			out, _, isRegister = c.quickCompileExpr(variable.Index, intType)
			if isRegister {
				index = out
			} else {
...
func (c *Compiler) compileAssignment(variables []ast.Expression, value ast.Expression, isDecl bool) {
	...
	if len(variables) == 1 { // TODO(Gianluca): remove this!
		...
		case *ast.Selector:
			if v, ok := c.availableVariables[variable.Expr.(*ast.Identifier).Name+"."+variable.Ident]; ok {
				index := c.variableIndex(v)
				out, _, isRegister := c.quickCompileExpr(value, typ)
				var tmpReg int8
				if isRegister {
					tmpReg = out
				} else {
		...
		case *ast.Index:
			...
			case reflect.Slice:
				var slice int8
				out, _, isRegister := c.quickCompileExpr(variable.Expr, c.typeinfo[variable].Type)
				if isRegister {
					slice = out
				} else {
					slice = c.fb.NewRegister(reflect.Interface)
				...
				var kvalue bool
				var valueReg int8
				out, isValue, isRegister := c.quickCompileExpr(value, exprType.Elem())
				if isValue {
					valueReg = out
					kvalue = true
				} else if isRegister {
...
func (c *Compiler) compileBuiltin(call *ast.Call, reg int8, dstType reflect.Type) {
	...
	case "copy":
		var dst, src int8
		out, _, isRegister := c.quickCompileExpr(call.Args[0], c.typeinfo[call.Args[0]].Type)
		if isRegister {
			dst = out
		} else {
		...
		out, _, isRegister = c.quickCompileExpr(call.Args[1], c.typeinfo[call.Args[1]].Type)
		if isRegister {
			src = out
		} else {
	...
	case "delete":
		...
		var mapp, key int8
		out, _, isRegister := c.quickCompileExpr(mapExpr, c.typeinfo[mapExpr].Type)
		if isRegister {
			mapp = out
		} else {
		...
		out, _, isRegister = c.quickCompileExpr(keyExpr, keyType)
		if isRegister {
			key = out
		} else {
	...
	case "make":
		...
		case reflect.Slice:
			...
			var len, cap int8
			var kLen, kCap bool
			out, isValue, isRegister := c.quickCompileExpr(lenExpr, intType)
			if isValue {
				len = out
				kLen = true
			} else if isRegister {
			...
			out, isValue, isRegister = c.quickCompileExpr(capExpr, intType)
			if isValue {
				cap = out
				kCap = true
			} else if isRegister {
	...
	case "panic":
		arg := call.Args[0]
		var reg int8
		out, _, isRegister := c.quickCompileExpr(arg, emptyInterfaceType)
		if isRegister {
			reg = out
		} else {
	...
	for _, node := range nodes {
		...
		case *ast.Assignment:
			...
			case len(node.Variables) == 1 && len(node.Values) == 1 && (node.Type == ast.AssignmentAddition || node.Type == ast.AssignmentSubtraction):
				...
				out, isValue, isRegister := c.quickCompileExpr(node.Values[0], typ)
				var y int8
				var ky bool
				if isValue {
					y = out
			...
			case len(node.Variables) == len(node.Values):
				if mayHaveDepencencies(node.Variables, node.Values) {
					...
					for i := range node.Variables {
						...
						out, isValue, isRegister := c.quickCompileExpr(node.Values[i], valueType)
						var value int8
						kvalue := false
						if isValue {
							value = out
...
func (c *Compiler) compileSwitch(node *ast.Switch) {
	...
	for i, cas := range node.Cases {
		...
		for _, caseExpr := range cas.Expressions {
			var ky bool
			var y int8
			out, isValue, isRegister := c.quickCompileExpr(caseExpr, typ)
			if isValue {
				ky = true
				y = out
			} else if isRegister {
