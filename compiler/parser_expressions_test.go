...
var exprTests = []struct {
	...
	{"1*x::(T)", ast.NewBinaryOperator(p(1, 2, 0, 7), ast.OperatorMultiplication,
		ast.NewBasicLiteral(p(1, 1, 0, 0), ast.IntLiteral, "1"),
		ast.NewGlobalAssertion(p(1, 3, 2, 7), ast.NewIdentifier(p(1, 3, 2, 2), "x"), ast.NewIdentifier(p(1, 7, 6, 6), "T"))),
	},
	{"!x::(T)", ast.NewUnaryOperator(p(1, 1, 0, 6), ast.OperatorNot,
		ast.NewGlobalAssertion(p(1, 2, 1, 6), ast.NewIdentifier(p(1, 2, 1, 1), "x"), ast.NewIdentifier(p(1, 6, 5, 5), "T"))),
	},
	{"x::(interface{})", ast.NewGlobalAssertion(p(1, 1, 0, 15),
		ast.NewIdentifier(p(1, 1, 0, 0), "x"),
		ast.NewInterface(p(1, 5, 4, 14))),
	},
	{"x::(T)*1", ast.NewBinaryOperator(p(1, 7, 0, 7), ast.OperatorMultiplication,
