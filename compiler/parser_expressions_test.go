...
var exprTests = []struct {
	...
	{"x::p.T", ast.NewGlobalAssertion(p(1, 1, 0, 5), ast.NewIdentifier(p(1, 1, 0, 0), "x"), ast.NewSelector(p(1, 6, 5, 5),
		ast.NewIdentifier(p(1, 4, 3, 3), "p"), "T"))},
	{"1*x::T", ast.NewBinaryOperator(p(1, 2, 0, 5), ast.OperatorMultiplication,
		ast.NewBasicLiteral(p(1, 1, 0, 0), ast.IntLiteral, "1"),
		ast.NewGlobalAssertion(p(1, 3, 2, 5), ast.NewIdentifier(p(1, 3, 2, 2), "x"), ast.NewIdentifier(p(1, 6, 5, 5), "T"))),
	},
	{"!x::T", ast.NewUnaryOperator(p(1, 1, 0, 4), ast.OperatorNot,
		ast.NewGlobalAssertion(p(1, 2, 1, 4), ast.NewIdentifier(p(1, 2, 1, 1), "x"), ast.NewIdentifier(p(1, 5, 4, 4), "T"))),
	},
	{"x::interface{}", ast.NewGlobalAssertion(p(1, 1, 0, 13),
		ast.NewIdentifier(p(1, 1, 0, 0), "x"),
		ast.NewInterface(p(1, 4, 3, 13))),
	},
	{"x::T*1", ast.NewBinaryOperator(p(1, 5, 0, 5), ast.OperatorMultiplication,
		ast.NewGlobalAssertion(p(1, 1, 0, 3), ast.NewIdentifier(p(1, 1, 0, 0), "x"), ast.NewIdentifier(p(1, 4, 3, 3), "T")),
		ast.NewBasicLiteral(p(1, 6, 5, 5), ast.IntLiteral, "1")),
	},
