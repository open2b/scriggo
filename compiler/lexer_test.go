...
var typeTests = map[string][]tokenTyp{
	...
	"{% include \"\" %}":                {tokenStartBlock, tokenInclude, tokenInterpretedString, tokenEndBlock},
	"{# comment #}":                     {tokenComment},
	"{# nested {# comment #} #}":        {tokenComment},
	"{# nested {## verbatim ##} #}":     {tokenComment},
	"{## verbatim ##}":                  {tokenVerbatim},
	"{## verbatim {# comment #} ##}":    {tokenVerbatim},
	"{## verbatim {## verbatim ##} ##}": {tokenVerbatim},
...
func TestLexerIndexCommentTag(t *testing.T) {
	tests := []struct {
		s string
		i int
	}{
	...
	for _, test := range tests {
		if i := indexCommentTag([]byte(test.s)); i != test.i {
			t.Errorf("source: %q, unexpected %d, expecting %d\n", test.s, i, test.i)
		}
	}
}

func TestLexerIndexVerbatimTag(t *testing.T) {
	tests := []struct {
		s string
		i int
	}{
	...
	for _, test := range tests {
		if i := indexVerbatimTag([]byte(test.s)); i != test.i {
			t.Errorf("source: %q, unexpected %d, expecting %d\n", test.s, i, test.i)
		}
	}
}
